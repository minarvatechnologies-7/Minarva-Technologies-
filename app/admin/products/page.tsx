...
export default function AdminProductsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (active) setError(err instanceof Error ? err.message : "Unable to load catalogue");
      }
    })();
    return () => { active = false; };
  }, [router]);
  ...
  return (
    <main className="portalPage">
      <header className="portalHeader"><Link className="brand" href="/admin">MINARVA<span>TECHNOLOGIES</span></Link><div><span>PRODUCT MANAGEMENT</span><Link className="textButton" href="/products">Public catalogue</Link></div></header>
      <section className="portalHero"><div><p className="eyebrow">CATALOGUE CONTROL</p><h1>Keep the product data real.</h1><p>Add verified products, specifications and used-device listings without inventing live stock or prices.</p></div></section>
      <section className="portalGrid">
        <article className="portalPanel"><div className="panelTitle"><h2>Add product</h2><span>Admin only</span></div><form onSubmit={submit}>
          ...
          {message && <div className="formSuccess">{message}</div>}{error && <div className="formError">{error}</div>}
          <button className="primary" type="submit">Save product <span>→</span></button>
        </form></article>
        <article className="portalPanel"><div className="panelTitle"><h2>Catalogue records</h2><span>{products.length} shown</span></div>{products.length ? products.map((product) => <div className="portalRow" key={product.id}><div><b>{product.name}</b><p>{product.category.name} · {product.sku || "No SKU"}{product.isUsed ? " · USED" : ""}</p></div><span>{product.active ? "ACTIVE" : "INACTIVE"}</span></div>) : <p className="muted">No products have been added yet.</p>}</article>
      </section>
    </main>
  );
}
