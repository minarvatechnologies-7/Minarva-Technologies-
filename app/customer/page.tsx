...
export default function CustomerPortal() {
  ...
  useEffect(() => {
    fetch("/api/customer/dashboard")
      ...
      .then(setData)
      .catch((err) => { setError(err instanceof Error ? err.message : "Unable to load portal"); if (err instanceof Error && err.message === "Authentication required") router.push("/login"); });
  }, [router]);
  ...
  return (
    <main className="portalPage">
      ...
      <section className="portalGrid">
        <article className="portalPanel"><div className="panelTitle"><h2>Service requests</h2><span>{data.customer.tickets.length}</span></div>{data.customer.tickets.length ? data.customer.tickets.map((ticket) => <div className="portalRow" key={ticket.id}><div><b>MN-SRV-{String(ticket.ticketNumber).padStart(6, "0")}</b><p>{ticket.serviceSlug}</p></div><span>{ticket.status.replaceAll("_", " ")}</span></div>) : <p className="muted">No service requests yet.</p>}</article>
        <article className="portalPanel"><div className="panelTitle"><h2>Quotations</h2><span>{data.customer.quotations.length}</span></div>{data.customer.quotations.length ? data.customer.quotations.map((quote) => <div className="portalRow" key={quote.id}><div><b>Quote #{quote.quoteNumber}</b><p>{new Date(quote.createdAt).toLocaleDateString()}</p></div><span>₹{Number(quote.total).toLocaleString("en-IN")}</span></div>) : <p className="muted">No quotations yet.</p>}</article>
