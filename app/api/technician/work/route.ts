...
export async function POST(request: Request) {
  ...
  if (!parsed.success || (!parsed.data.status && !parsed.data.note)) return NextResponse.json({ ok: false, error: "Provide a valid status or work note" }, { status: 400 });

  const current = await prisma.serviceTicket.findFirst({ where: { id: parsed.data.ticketId, technicianId: auth.user.id }, select: { id: true, status: true, ticketNumber: true } });
  ...
  const updated = await prisma.serviceTicket.update({
    ...
    data: {
      ...
      timeline: { create: { status: nextStatus, note: parsed.data.note || null, photoUrls: [] } },
    },
