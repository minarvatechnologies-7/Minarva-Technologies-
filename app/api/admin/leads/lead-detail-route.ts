...
export async function PATCH(request: Request, { params }: { params: Promise<{ leadId: string }> }) {
  ...
  const current = await prisma.lead.findUnique({ where: { id: leadId }, select: { id: true, status: true, temperature: true, assignedToId: true } });
  ...
  const updated = await prisma.lead.update({ where: { id: leadId }, data: { status: parsed.data.status, temperature: parsed.data.temperature, assignedToId: parsed.data.assignedToId === undefined ? undefined : parsed.data.assignedToId, followUpAt: parsed.data.followUpAt === undefined ? undefined : (parsed.data.followUpAt ? new Date(parsed.data.followUpAt) : null), activities: changes.length ? { create: { userId: auth.user.id, type: "LEAD_UPDATED", body: changes.join(" | ") } } : undefined }, select: { id: true, leadNumber: true, status: true, temperature: true, score: true, assignedTo: { select: { id: true, name: true, role: true } }, followUpAt: true, updatedAt: true } });
